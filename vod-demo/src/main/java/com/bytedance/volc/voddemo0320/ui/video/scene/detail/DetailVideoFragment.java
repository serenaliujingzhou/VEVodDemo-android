...
package com.bytedance.volc.voddemo0320.ui.video.scene.detail;
...
import com.bytedance.playerkit.player.utils.TestUtil;
...
//            mMediaSource = (MediaSource) bundle.getSerializable(EXTRA_MEDIA_SOURCE);
            String url = TestUtil.getNextPlayUrl();
            if (TextUtils.isEmpty(url)) {
                Toast.makeText(this.getContext(), "没有视频了！", Toast.LENGTH_SHORT).show();
                return;
            }
            VideoItem videoItem = VideoItem.createUrlItem(url, null);
            mMediaSource = VideoItem.toMediaSource(videoItem);
            mContinuesPlayback = bundle.getBoolean(EXTRA_CONTINUES_PLAYBACK);
        }
    }

    private void initNextVideoButton(@NonNull View view) {
        mBtnNext = view.findViewById(R.id.btn_next_video);
        mBtnNext.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String url = TestUtil.getNextPlayUrl();
                if (TextUtils.isEmpty(url)) {
                    Toast.makeText(view.getContext(), "没有视频了！", Toast.LENGTH_SHORT).show();
                    return;
                }
                VideoItem videoItem = VideoItem.createUrlItem(url, null);
                mMediaSource = VideoItem.toMediaSource(videoItem);
        ...
        mFirstRenderTime = view.findViewById(R.id.first_render_cost_time);
    }
    ...
    private VideoView createVideoView(Context context) {
        ...
        videoView.controller().addPlaybackListener(mPlaybackListener);
        return videoView;
    }
        ...
        public void onEvent(Event event) {
            switch (event.code()) {
                case PlayerEvent.Info.VIDEO_RENDERING_START:
                    TestUtil.startRender();
                    mFirstRenderTime.setText(TestUtil.costTimeStr);
//                    if (TestUtil.isFirst) {
//                        mVideoView.stopPlayback();
//                        TestUtil.isFirst = false;
//                    } else {
                        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                            ...
                            public void run() {
                                String url = TestUtil.getNextPlayUrl();
                                if (TextUtils.isEmpty(url)) {
                                    Toast.makeText(getContext(), "没有视频了！", Toast.LENGTH_SHORT).show();
                                    return;
                                }
                                VideoItem videoItem = VideoItem.createUrlItem(url, null);
                                mMediaSource = VideoItem.toMediaSource(videoItem);
